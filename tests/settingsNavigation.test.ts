...
describe('settings navigation structure', () => {
  it('defines the fixed app pages used by the sidebar navigation', async () => {
    ...
    expect(app).toContain('有失败录音可重新处理');
    expect(app).toContain('重新处理');
    expect(app).toContain('删除录音');
    expect(app).toContain('getRecoveryJobs');
    expect(app).toContain('retryRecoveryJob');
    expect(app).toContain('deleteRecoveryJob');
    expect(app).toContain('copyRecoveryDiagnostic');
    ...
    expect(app).toContain('control-chip');
    expect(app).toContain('settings-field');
    expect(app).toContain('choice-group compact');
    expect(app).toContain('settings-diagnostics-toolbar');
    expect(app).toContain('voice-diagnostics-toolbar');
    expect(app).toContain('当前 ASR 后端');
    ...
    expect(styles).toContain('.recovery-panel');
    ...
    expect(styles).toContain('.control-chip');
    expect(styles).toContain('.settings-field');
    expect(styles).toContain('.choice-group');
    expect(styles).toContain('.settings-diagnostics-toolbar');
    expect(styles).toMatch(/\.settings-field,[\s\S]*?\.control-chip,[\s\S]*?\.settings-control-strip\s*\{[^}]*min-height:\s*var\(--control-height\)/s);
    ...
    expect(styles).toContain('.control-action');
    expect(styles).toContain('.history-text');
    ...
    expect(design).toContain('choice-group:');
    expect(design).toContain('history-text:');
    expect(design).toContain('dense-table:');
    ...
    expect(design).toContain('短选项组使用扁平 `choice-group`');
    expect(design).toContain('历史正文、统计表和词库编辑器属于高频扫描区域');
  });
});
